    ...
    <div className="space-y-24">
      {MainContent.map((content, index) => (
        <div key={content.title} className="sticky top-24" style={{ zIndex: index + 1 }}>
          <div className="relative rounded-3xl bg-[#111] px-10 py-12">
            <div className="grid grid-cols-2 gap-20 items-start">
              <div>
                <h2 className="text-9xl font-avenir-light!">0{index + 1}</h2>
              </div>
              <div>
                <div className="w-[80%]">
                  <h4 className="text-3xl mb-2">{content.title}</h4>
                  <p className="text-2xl text-white font-avenir-light mb-8!">
                    {content.desc}
                  </p>
                  <Image
                    src={content.img}
